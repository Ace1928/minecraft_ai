...
- [ ] Requalify cold Bedrock startup after the launcher permits a new GPU session. On 2026-09-08, the 30-second geometry preflight expired before finding one exact viewable Minecraft window and its cleanup interrupted startup. The subsequent bounded diagnostic was refused by BedrockOnLinux's current-boot GPU safety guard before Wine started. The closed world was backed up and remained byte-identical; no post-restart camera or gameplay result exists. Inspect the startup window timeline and startup budget after the required reboot; do not bypass the GPU guard or mistake a longer timeout for a proven geometry fix.
