...
- [ ] Demonstrate ordered operator inventory-command completion: open, verify, then close. The correction and first ordinary-instruction trials produced no-op close decisions without opening inventory. After `6ab3332`, the same ordinary instruction selected `open_inventory`: run `4e4bd43cd8b6459a91b1d5a031e5cfbb` emitted accepted E at sequence 62, and retained steps 60/61 visibly show inventory. However, the populated recipe pane failed the deterministic GUI classifier; no `scene.*` facts were published, the skill timed out after 10.008 seconds, and the plan did not advance. The first 120 seconds contained no close/Escape, locomotion key presses or attacks, but 30 camera-motion packets followed keepalive resumption. A later repeated attempt and root retirement/archive are not command completion. Reliable sequencing, GUI recognition and paid-instruction admission remain open; acknowledgment is not completion.
