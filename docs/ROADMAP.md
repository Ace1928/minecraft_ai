...
- [ ] Jointly qualify the configured native worker and then activate the gameplay upgrade through an agent-only reload. Preserve the outer launcher shell-only barrier, quiescent recovery children, exact supervisor generation, game, calibration and GPU process. The currently loaded legacy worker needs its own observed first-transition shutdown/checkpoint/reap evidence, not an invented new-protocol ACK. Retain separate checkpoint, recorder, containment and retained-image gameplay outcomes. The live agent has not been reloaded for the new transport/caller changes.
