...
- [x] Reject external evaluation evidence that overwrites measured metrics or uses reserved computed namespaces, including absent duration/latency keys. A no-op recording plus a destination event still fails measured forward movement; ordinary outcome and unscored behavior are retained. Mutable-evidence bypass and non-finite outcomes reject, and CLI errors precede report/database writes. See [evaluation contracts and closed-finding evidence](EVALUATION.md); no gameplay or promotion result is implied.
- [x] Reject unknown capture-source preferences during parent CLI parsing before any runtime mutation. Parent, child and factory share one enum; factory rejection precedes backend access even without a host binding. Regressions retain both supported sources, the headless X11 fallback for a valid PipeWire preference, and exact host routing. Operator documentation now describes headless navigation and command-origin calibration accurately; physical horizon/gameplay qualification remains open.
- [x] Add optional `run --config` selection through the existing agent launch path. Explicit files are resolved and parsed before attachment or arming; calibration and child launch share the selected configuration without rewriting the default. Invalid-file, calibration, pause, launch-transaction and child-argument regressions qualify the source behavior. This enables separately managed adapter configurations, not a live replacement or immutable-source guarantee; external watchdog recovery still needs its existing owner guard and explicit configuration selection.
- [x] Make the existing missing-question repair failure-aware for versioned skills with resource prerequisites. After a typed locomotion stall in the same unfinished autonomous role plan, a null repair that requests replanning but supplies no canonical question asks only `obstacle.ahead`; it still authorizes no action. Valid model questions, operator contexts, other requests, mismatched plans and non-stall failures keep their previous behavior. The 118-test cognition contract suite passes. The current bootstrap and saved gather definitions have no resource prerequisites, and retained live records do not expose the pre-augmentation model questions: this qualifies a source path, not the cause or resolution of the observed live target-query loop.
...
- [x] Wire terminal worker drain into planned runtime retirement at source level. Actual supervisor release precedes reset-free/action-free skill cancellation and checkpointing; native/legacy workers remain distinct, all worker waits share one cutoff with recorder reserve, and failed cleanup cannot skip later owners. Normal supervisor stop signals the verified parent first; bounded, identity-rechecked group containment remains separate from save evidence. Focused fake-worker/runtime/process tests cover deadlines, stale owners, late perception publication and retryable recorder closure. This is source qualification, not a live reload or native gameplay result.
- [ ] Jointly qualify the configured native worker and its gameplay upgrade through an agent-only reload. Preserve the outer launcher shell-only barrier, quiescent recovery children, exact supervisor generation, game, calibration and GPU process. Evaluate the first legacy generation's shutdown/checkpoint/reap from its actual evidence, not an invented new-protocol ACK. Retain separate checkpoint, recorder, containment and retained-image gameplay outcomes. Successful configured-native startup and action-backed gameplay qualification remain open.
- Live transition update: one bounded private startup trial now retired its candidate and restored a healthy, separately configured replacement using the qualified public/native sources. The default configuration, game client, supervisor session, calibration and GPU model were preserved, and the original launcher resumed after replacement readiness. The candidate's sealed recording contains zero accepted inputs and zero drops: its native process emitted a delegation marker followed by a generic startup failure, before readiness admission. That marker does not distinguish native initialization from READY validation; source inspection found no proven schema mismatch. This is a failed candidate startup with successful replacement restoration, not native gameplay, checkpoint-handshake, exact state-continuity or learning proof. No retry was performed.
- Recorded qualification update: one CPU-only, saved-frame attempt completed in 80.71 seconds with two normal worker exits. Its report records matching recurrent/RNG state at the 24/48-frame checkpoints and after the final reset, but the overall gate **failed**: only 46 of 48 inference requests returned within the unchanged 400 ms deadline; the two expired requests emitted no action. The parent therefore rejected the candidate before its final acceptance validator. This is useful component evidence, not a qualified live replacement, an online-learning result or improved survival. The existing live game, agent and recorder were preserved; no retry or reload was performed.
- Recorded qualification follow-up: a separately authorized diagnostic attempt now passes the original gate: 48/48 predictions matched the sequential native reference and arrived before 400 ms, with exact 24/48 and final-reset state/RNG, both normal checkpointed worker exits and clean parent/worker/tracker/terminal receipts. It completed in 64.84 seconds; the slowest reply was 319.70 ms. The sole behavior-file change retains already-measured per-request timings in the final report; it is not a model optimization. The earlier failure remains retained and its timing cause unresolved. This qualifies that bounded recorded execution, not sustained live timing, improved gameplay, online learning or the still-pending agent-only reload.
