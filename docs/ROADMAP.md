...
- [x] Carry an explicit planner target description into its existing requested-perception transaction, including format repair. Missing descriptions are explicitly unresolved in the question; no target is inferred from reasoning, past skills or plans. Typed requests cannot acquire unrelated chat evidence from descriptor wording. The 406 focused regressions preserve scene ownership, preemption, action restrictions and the obstacle-only idle-stall fallback. This is a question-delivery fix, not verified target recognition or acquisition.
