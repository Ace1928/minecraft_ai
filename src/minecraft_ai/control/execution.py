...
from minecraft_ai.mining_control import MiningLeaseGuard, is_rocket_source, track_contains_crosshair
from minecraft_ai.motor import MotorIntent, MotorPolicy
...
class SkillExecutor:
    ...
    def _tick_death_respawn(
        self,
        *,
        sequence: int,
        capture: CapturedFrame | None,
    ) -> ExecutionTick:
        """Click the visible Respawn control once, then wait for a playable HUD.

        Death recovery is a screenshot-bound GUI click, not a learned search.
        ...
        if capture is None:
            return self._finish(
                SkillOutcome.FAILED,
                time.monotonic_ns(),
                "death-respawn-frame-missing",
                recover=True,
            )
        point = death_respawn_control_center(capture)
        if point is None:
            return self._finish(
                SkillOutcome.FAILED,
                time.monotonic_ns(),
                "death-respawn-control-unverified",
                recover=True,
            )
        cursor_x, cursor_y = point
        self._death_respawn_sent = True
        return ExecutionTick(
            run=self._run,
            action=MotorAction(
                sequence=sequence,
