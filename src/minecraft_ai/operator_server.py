...
$('instruction').textContent=esc(t.active_instruction||'Waiting for an executable option.');const p=t.policy||{},active=p.active_route==='grounded'?(p.grounded||p):(p.primary||p),pred=active.last_prediction||{},counts=active.learned_action_counts||{};$('policy').textContent=esc(active.model_version||p.policy_id);$('policyMetrics').textContent=(active.last_inference_ms??'—')+' ms · '+esc(p.active_route||active.grounding_mode)+' · '+esc(active.accepted_predictions||0)+' learned · '+esc(counts.jump||0)+' jumps · '+esc(counts.camera||0)+' camera · target '+(pred.target_exists_probability==null?'—':Math.round(pred.target_exists_probability*100)+'%');
