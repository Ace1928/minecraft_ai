...
from minecraft_ai.models import VisionLanguageModel
from .types import (
...
# (x0, x1, y0, y1) in normalized full-frame coordinates. The first pair is the
# original 1279x635 fixture. The second is the isolated 1920x1080 Bedrock
# session: Respawn sits near y=0.66-0.73, Game Menu near y=0.75-0.82.
_DEATH_CONTROL_PAIRS: tuple[
    tuple[tuple[float, float, float, float], tuple[float, float, float, float]],
    ...,
] = (
    ((0.39, 0.61, 0.77, 0.81), (0.39, 0.61, 0.85, 0.89)),
    ((0.34, 0.66, 0.66, 0.73), (0.34, 0.66, 0.75, 0.82)),
)
...
def bedrock_death_screen_present(frame: CapturedFrame) -> bool:
    ...
    this a conservative negative-only scene interlock. It does not emit an
    action, and it is not a training label.
    """
    return _matching_death_primary_band(frame) is not None


def death_respawn_control_center(frame: CapturedFrame) -> tuple[float, float] | None:
    """Return the normalized center of the green Respawn control, if present.

    The point is derived from the same paired-control geometry as
    ``bedrock_death_screen_present``. It is a screenshot-bound GUI target, not
    a world-state oracle.
    """
    band = _matching_death_primary_band(frame)
    if band is None:
        return None
    x0, x1, y0, y1 = band
    pixels = _numpy_bgra(frame)
    if pixels is None:
        return ((x0 + x1) / 2.0, (y0 + y1) / 2.0)
    x_start, x_end = int(frame.width * x0), int(frame.width * x1)
    y_start, y_end = int(frame.height * y0), int(frame.height * y1)
    roi = pixels[y_start:y_end, x_start:x_end, :3]
    if roi.size == 0:
        return None
    blue, green, red = roi[:, :, 0], roi[:, :, 1], roi[:, :, 2]
    ...
    if not bool(selected.any()):
        return ((x0 + x1) / 2.0, (y0 + y1) / 2.0)
    ys, xs = selected.nonzero()
    return (
        (x_start + float(xs.mean())) / float(frame.width),
        (y_start + float(ys.mean())) / float(frame.height),
    )


def _matching_death_primary_band(
    frame: CapturedFrame,
) -> tuple[float, float, float, float] | None:
    if not frame.bgra or frame.width < 64 or frame.height < 64:
        return None
    pixels = _numpy_bgra(frame)
    for primary, secondary in _DEATH_CONTROL_PAIRS:
        if _death_control_pair_present(frame, primary, secondary, pixels=pixels):
            return primary
    return _scan_death_primary_band(frame, pixels=pixels)


def _death_control_pair_present(
    frame: CapturedFrame,
    ...
    *,
    ...
    px0, px1, py0, py1 = primary
    sx0, sx1, sy0, sy1 = secondary
...
def _scan_death_primary_band(
    ...
    *,
    ...
    """Accept a centered green control with a gray control immediately below.

    GUI scale moves the death pair vertically. Keep the search in the lower
    half and require both palettes so grass or stone cannot authorize death.
    """
    x0, x1 = 0.34, 0.66
    window = 0.04
    for index in range(13):
        primary_y0 = 0.52 + index * 0.02
        primary_y1 = primary_y0 + window
        if primary_y1 > 0.84:
            break
        secondary_y0 = primary_y1 + 0.04
        secondary_y1 = secondary_y0 + window
        if secondary_y1 > 0.94:
            continue
        primary = (x0, x1, primary_y0, primary_y1)
        secondary = (x0, x1, secondary_y0, secondary_y1)
        if _death_control_pair_present(frame, primary, secondary, pixels=pixels):
            return primary
    return None
